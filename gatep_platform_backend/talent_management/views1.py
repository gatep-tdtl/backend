...
# class ResumeAIPipeline:
#     # This class remains unchanged. Its logic is sound for extraction.
#     def __init__(self):
#         self.client = InferenceClient(_A, token=HFF_TOKEN)
#         self.temp_pdf_paths = []
#     def _extract_text_from_pdf(self, path):
...
#             doc.close()
#         except Exception as e: raise Exception(f"Error extracting text from PDF: {e}. Ensure it's a valid PDF.")
#         return text
#     def _build_prompt(self, form_data, pdf_text):
#         form_info_str = json.dumps(form_data, indent=2)
#         return f'''
# You are an AI Resume Builder...
# ...
# ... [The entire long prompt is unchanged and correct]
# ...
# Strict JSON Output:
# '''
#     def _extract_score_from_marksheet_text(self, marksheet_text, education_level):
...
#             response = self.client.chat.completions.create(model=_A, messages=[{'role': 'user', _B: prompt}], max_tokens=50, temperature=.1, stop=['\n'])
#             score = response.choices[0].message.content.strip()
#             if re.match('^[0-9.]+\\s*(%|CGPA|GPA|Grade)?$', score, re.IGNORECASE): return score
#             else: return ''
...
#                 try:
#                     with open(marksheet_pdf_path, 'wb+') as temp_file:
#                         for chunk in upload_file.chunks(): temp_file.write(chunk)
#                     self.temp_pdf_paths.append(marksheet_pdf_path)
#                     marksheet_text = self._extract_text_from_pdf(marksheet_pdf_path)
#                     extracted_score = self._extract_score_from_marksheet_text(marksheet_text, level)
#                     if extracted_score:
...
#                         if level not in structured_resume[A]: structured_resume[A][level] = {}
#                         structured_resume[A][level][D] = extracted_score
#                 except Exception as score_extract_e: pass
#         return structured_resume
#     def get_temp_pdf_paths(self):
#         return self.temp_pdf_paths
#     def _populate_structured_resume_from_form(self, data_for_llm_prompt):
...
#         structured_resume = {}
#         structured_resume[B] = { L: data_for_llm_prompt.get(L, ''), M: data_for_llm_prompt.get(M, ''), N: data_for_llm_prompt.get(N, ''), S: data_for_llm_prompt.get(S, ''), T: data_for_llm_prompt.get(T, ''), U: data_for_llm_prompt.get(U, ''), V: data_for_llm_prompt.get(V, ''), "current_area": data_for_llm_prompt.get("current_area", ""), "permanent_area": data_for_llm_prompt.get("permanent_area", ""), "current_city": data_for_llm_prompt.get("current_city", ""), "permanent_city": data_for_llm_prompt.get("permanent_city", ""), "current_district": data_for_llm_prompt.get("current_district", ""), "permanent_district": data_for_llm_prompt.get("permanent_district", ""), "current_state": data_for_llm_prompt.get("current_state", ""), "permanent_state": data_for_llm_prompt.get("permanent_state", ""), "current_country": data_for_llm_prompt.get("current_country", ""), "permanent_country": data_for_llm_prompt.get("permanent_country", "") }
#         structured_resume[I] = { W: data_for_llm_prompt.get(W, ''), X: data_for_llm_prompt.get(X, ''), Y: data_for_llm_prompt.get(Y, ''), Z: data_for_llm_prompt.get(Z, ''), a: data_for_llm_prompt.get(a, '') }
#         structured_resume[s] = data_for_llm_prompt.get('summary', '')
#         structured_resume[h] = data_for_llm_prompt.get(h, [])
#         structured_resume[i] = data_for_llm_prompt.get(i, [])
#         structured_resume[j] = data_for_llm_prompt.get(j, [])
#         structured_resume[k] = data_for_llm_prompt.get(k, [])
#         structured_resume[l] = data_for_llm_prompt.get(l, [])
#         structured_resume[m] = data_for_llm_prompt.get(m, [])
#         structured_resume[n] = data_for_llm_prompt.get(n, [])
#         structured_resume[d] = data_for_llm_prompt.get(d, '')
#         structured_resume[f] = data_for_llm_prompt.get(f, '')
#         structured_resume[p] = data_for_llm_prompt.get(p, {})
#         structured_resume[o] = data_for_llm_prompt.get(o, [])
#         structured_resume[q] = data_for_llm_prompt.get(q, [])
#         raw_preferences = data_for_llm_prompt.get(E, {})
#         if not isinstance(raw_preferences, dict):
#             try: raw_preferences = json.loads(raw_preferences)
#             except Exception: raw_preferences = {}
#         if not isinstance(raw_preferences, dict): raw_preferences = {}
#         structured_resume[E] = raw_preferences
#         structured_resume[E][O] = data_for_llm_prompt.get(O, '')
#         structured_resume[E][P] = data_for_llm_prompt.get(P, '')
#         structured_resume[Q] = data_for_llm_prompt.get(Q, '')
#         structured_resume[R] = data_for_llm_prompt.get(R, '')
#         structured_resume[v] = { b: data_for_llm_prompt.get(b, ''), c: data_for_llm_prompt.get(c, '') }
#         structured_resume['document_verification'] = data_for_llm_prompt.get('document_verification', 'Incomplete')
#         structured_resume[A] = { F: {t: data_for_llm_prompt.get('tenth_board_name', ''), 'school_name': data_for_llm_prompt.get('tenth_school_name', ''), K: data_for_llm_prompt.get('tenth_year_passing', ''), D: data_for_llm_prompt.get('tenth_score', '')}, G: {t: data_for_llm_prompt.get('twelfth_board_name', ''), 'college_name': data_for_llm_prompt.get('twelfth_college_name', ''), K: data_for_llm_prompt.get('twelfth_year_passing', ''), D: data_for_llm_prompt.get('twelfth_score', '')}, H: {'course_name': data_for_llm_prompt.get('diploma_course_name', ''), u: data_for_llm_prompt.get('diploma_institution_name', ''), K: data_for_llm_prompt.get('diploma_year_passing', ''), D: data_for_llm_prompt.get('diploma_score', '')}, C: {g: data_for_llm_prompt.get('degree_name', ''), u: data_for_llm_prompt.get('degree_institution_name', ''), 'specialization': data_for_llm_prompt.get('degree_specialization', ''), K: data_for_llm_prompt.get('degree_year_passing', ''), D: data_for_llm_prompt.get('degree_score', '')} }
#         return structured_resume
...
#     def _process_incoming_form_data(self, request):
#         # This method remains unchanged from the last version. It correctly parses all fields.
#         data_for_llm_prompt = {}
#         files = {}
#         plain_text_fields = [L, M, N, 'summary', O, P, Q, R, S, T, U, V, W, X, Y, Z, a, b, c, d, f, 'tenth_board_name', 'tenth_school_name', 'tenth_year_passing', 'tenth_score', 'twelfth_board_name', 'twelfth_college_name', 'twelfth_year_passing', 'twelfth_score', 'diploma_course_name', 'diploma_institution_name', 'diploma_year_passing', 'diploma_score', g, 'degree_institution_name', 'degree_specialization', 'degree_year_passing', 'degree_score', 'document_verification']
#         for field in plain_text_fields:
#             if field in request.data: data_for_llm_prompt[field] = request.data.get(field)
#         all_json_fields = [h, i, j, k, l, m, n, o, q, p, 'frameworks_tools', 'diploma_details', 'degree_details', 'certification_details', 'certification_photos', 'work_preferences', 'work_authorizations', 'professional_links']
#         for field in all_json_fields:
#             if field in request.data:
#                 json_str_val = request.data.get(field)
#                 if json_str_val and isinstance(json_str_val, str):
#                     try: data_for_llm_prompt[field] = json.loads(json_str_val)
#                     except json.JSONDecodeError: data_for_llm_prompt[field] = [json_str_val] if field not in [p] else {}
#                 elif json_str_val: data_for_llm_prompt[field] = json_str_val
#                 else: data_for_llm_prompt[field] = {} if field in [p] else []
#         if E in request.data:
#             pref_json_str = request.data.get(E)
#             if pref_json_str:
#                 try: data_for_llm_prompt[E] = json.loads(pref_json_str)
#                 except json.JSONDecodeError: data_for_llm_prompt[E] = pref_json_str
#             else: data_for_llm_prompt[E] = {}
#         location_fields = ['current_area', 'permanent_area', 'current_city', 'permanent_city', 'current_district', 'permanent_district', 'current_state', 'permanent_state', 'current_country', 'permanent_country']
#         for field in location_fields:
#             if field in request.data: data_for_llm_prompt[field] = request.data.get(field)
#         file_fields = ['resume_pdf', 'profile_photo', 'tenth_result_upload', 'twelfth_result_upload', 'diploma_result_upload', 'degree_result_upload']
#         for field in file_fields:
#             if field in request.data: files[field] = request.data.get(field)
#         return {'data_for_llm_prompt': data_for_llm_prompt, 'files': files}
...
#         resume_instance.email = structured_resume.get(B, {}).get(M, resume_instance.email)
#         resume_instance.phone = structured_resume.get(B, {}).get(N, resume_instance.phone)
#         # ... [rest of the simple field assignments are correct] ...
#         resume_instance.current_location = structured_resume.get(B, {}).get(S, resume_instance.current_location)
#         resume_instance.current_company = structured_resume.get(B, {}).get(V, resume_instance.current_company)
#         resume_instance.aadhar_number = structured_resume.get(B, {}).get(T, resume_instance.aadhar_number)
#         resume_instance.passport_number = structured_resume.get(B, {}).get(U, resume_instance.passport_number)
...
#         resume_instance.summary = structured_resume.get(s, resume_instance.summary)
#         resume_instance.generated_summary = structured_resume.get(s, resume_instance.generated_summary)
#         if E in structured_resume: resume_instance.preferences = json.dumps(structured_resume[E])
#         resume_instance.work_arrangement = structured_resume.get(E, {}).get(O, resume_instance.work_arrangement)
#         resume_instance.preferred_location = structured_resume.get(E, {}).get(P, resume_instance.preferred_location)
#         resume_instance.preferred_tech_stack = structured_resume.get(Q, resume_instance.preferred_tech_stack)
#         resume_instance.dev_environment = structured_resume.get(R, resume_instance.dev_environment)
...
#         # Unchanged TextField assignments
#         if h in structured_resume: resume_instance.skills = json.dumps(structured_resume.get(h, []))
#         if i in structured_resume: resume_instance.experience = json.dumps(structured_resume.get(i, []))
#         if j in structured_resume: resume_instance.projects = json.dumps(structured_resume.get(j, []))
#         if k in structured_resume: resume_instance.certifications = json.dumps(structured_resume.get(k, []))
#         if l in structured_resume: resume_instance.awards = json.dumps(structured_resume.get(l, []))
#         if m in structured_resume: resume_instance.publications = json.dumps(structured_resume.get(m, []))
#         if n in structured_resume: resume_instance.open_source_contributions = json.dumps(structured_resume.get(n, []))
#         if o in structured_resume: resume_instance.interests = json.dumps(structured_resume.get(o, []))
#         if q in structured_resume: resume_instance.references = json.dumps(structured_resume.get(q, []))
#         if p in structured_resume: resume_instance.languages = json.dumps(structured_resume.get(p, {}))
#         if 'frameworks_tools' in structured_resume: resume_instance.frameworks_tools = json.dumps(structured_resume.get('frameworks_tools', []))
#         if 'certification_details' in structured_resume: resume_instance.certification_details = json.dumps(structured_resume.get('certification_details', []))
#         if 'certification_photos' in structured_resume: resume_instance.certification_photos = json.dumps(structured_resume.get('certification_photos', []))
#         if 'work_preferences' in structured_resume: resume_instance.work_preferences = json.dumps(structured_resume.get('work_preferences', []))
#         if 'work_authorizations' in structured_resume: resume_instance.work_authorizations = json.dumps(structured_resume.get('work_authorizations', []))
#         if 'professional_links' in structured_resume: resume_instance.professional_links = json.dumps(structured_resume.get('professional_links', []))
        
#         # ----- START OF THE FIX -----
#         # Manually construct `diploma_details` and `degree_details` to ensure they are saved.
        
#         edu_details = structured_resume.get(A, {})

#         # 1. Handle Diploma Details
#         # Prioritize a directly provided list, but fall back to constructing it.
#         if 'diploma_details' in structured_resume and structured_resume['diploma_details']:
#              resume_instance.diploma_details = json.dumps(structured_resume['diploma_details'])
#         elif edu_details.get(H) and edu_details.get(H).get('course_name'):
#              # Construct the list from the education_details block
#              diploma_list = [{
#                 'course_name': edu_details.get(H, {}).get('course_name', ''),
#                 'institution_name': edu_details.get(H, {}).get(u, ''),
#                 'year_passing': edu_details.get(H, {}).get(K, ''),
#                 'score': edu_details.get(H, {}).get(D, '')
#              }]
#              resume_instance.diploma_details = json.dumps(diploma_list)
        
#         # 2. Handle Degree Details
#         # Prioritize a directly provided list, but fall back to constructing it.
#         if 'degree_details' in structured_resume and structured_resume['degree_details']:
#             resume_instance.degree_details = json.dumps(structured_resume['degree_details'])
#         elif edu_details.get(C) and edu_details.get(C).get(g):
#             # Construct the list from the education_details block
#             degree_list = [{
#                 'degree_name': edu_details.get(C, {}).get(g, ''),
#                 'institution_name': edu_details.get(C, {}).get(u, ''),
#                 'specialization': edu_details.get(C, {}).get('specialization', ''),
#                 'year_passing': edu_details.get(C, {}).get(K, ''),
#                 'score': edu_details.get(C, {}).get(D, '')
#             }]
#             resume_instance.degree_details = json.dumps(degree_list)

#         # ----- END OF THE FIX -----
        
#         # Free text and file fields (unchanged)
#         resume_instance.volunteering_experience = structured_resume.get(d, resume_instance.volunteering_experience)
#         resume_instance.extracurriculars = structured_resume.get(f, resume_instance.extracurriculars)
#         if files.get('profile_photo'): resume_instance.profile_photo = files.get('profile_photo')
#         elif 'profile_photo' in files and files.get('profile_photo') is None: resume_instance.profile_photo = None
#         if files.get('resume_pdf'): resume_instance.resume_pdf = files.get('resume_pdf')
#         elif 'resume_pdf' in files and files.get('resume_pdf') is None: resume_instance.resume_pdf = None

#         # Individual education fields (unchanged, this is still necessary)
#         if F in edu_details:
#             tenth_data = edu_details[F]
#             resume_instance.tenth_board_name = tenth_data.get(t, resume_instance.tenth_board_name)
#             resume_instance.tenth_school_name = tenth_data.get('school_name', resume_instance.tenth_school_name)
#             resume_instance.tenth_year_passing = tenth_data.get(K, resume_instance.tenth_year_passing)
#             resume_instance.tenth_score = tenth_data.get(D, resume_instance.tenth_score)
#         if files.get('tenth_result_upload'): resume_instance.tenth_result_upload = files.get('tenth_result_upload')
#         if G in edu_details:
#             twelfth_data = edu_details[G]
#             resume_instance.twelfth_board_name = twelfth_data.get(t, resume_instance.twelfth_board_name)
#             resume_instance.twelfth_college_name = twelfth_data.get('college_name', resume_instance.twelfth_college_name)
#             resume_instance.twelfth_year_passing = twelfth_data.get(K, resume_instance.twelfth_year_passing)
#             resume_instance.twelfth_score = twelfth_data.get(D, resume_instance.twelfth_score)
#         if files.get('twelfth_result_upload'): resume_instance.twelfth_result_upload = files.get('twelfth_result_upload')
#         if H in edu_details:
#             diploma_data = edu_details[H]
#             resume_instance.diploma_course_name = diploma_data.get('course_name', resume_instance.diploma_course_name)
#             resume_instance.diploma_institution_name = diploma_data.get(u, resume_instance.diploma_institution_name)
#             resume_instance.diploma_year_passing = diploma_data.get(K, resume_instance.diploma_year_passing)
#             resume_instance.diploma_score = diploma_data.get(D, resume_instance.diploma_score)
#         if files.get('diploma_result_upload'): resume_instance.diploma_result_upload = files.get('diploma_result_upload')
#         if C in edu_details:
#             degree_data = edu_details[C]
#             resume_instance.degree_name = degree_data.get(g, resume_instance.degree_name)
#             resume_instance.degree_institution_name = degree_data.get(u, resume_instance.degree_institution_name)
...
#             resume_instance.degree_year_passing = degree_data.get(K, resume_instance.degree_year_passing)
#             resume_instance.degree_score = degree_data.get(D, resume_instance.degree_score)
#         if files.get('degree_result_upload'): resume_instance.degree_result_upload = files.get('degree_result_upload')

#     def _serialize_resume_to_json(self, resume_instance):
#         # This method is unchanged from the last version. It correctly uses safe_json_loads.
...
#         data = {
#             'id': resume_instance.pk, 'talent_id': resume_instance.talent_id.pk, 'name': resume_instance.name, 'email': resume_instance.email, 'phone': resume_instance.phone, 'profile_photo_url': resume_instance.profile_photo.url if resume_instance.profile_photo else None, 'current_location': resume_instance.current_location, 'current_company': resume_instance.current_company, 'aadhar_number': resume_instance.aadhar_number, 'passport_number': resume_instance.passport_number, 'linkedin_url': resume_instance.linkedin_url, 'github_url': resume_instance.github_url, 'portfolio_url': resume_instance.portfolio_url, 'stackoverflow_url': resume_instance.stackoverflow_url, 'medium_or_blog_url': resume_instance.medium_or_blog_url, 'summary': resume_instance.summary, 'generated_summary': resume_instance.generated_summary, 'volunteering_experience': resume_instance.volunteering_experience, 'extracurriculars': resume_instance.extracurriculars, 'work_arrangement': resume_instance.work_arrangement, 'preferred_location': resume_instance.preferred_location, 'preferred_tech_stack': resume_instance.preferred_tech_stack, 'dev_environment': resume_instance.dev_environment, 'work_authorization': resume_instance.work_authorization, 'criminal_record_disclosure': resume_instance.criminal_record_disclosure, 'document_verification': resume_instance.document_verification,
#             'skills': safe_json_loads(resume_instance.skills, []), 'experience': safe_json_loads(resume_instance.experience, []), 'projects': safe_json_loads(resume_instance.projects, []), 'certifications': safe_json_loads(resume_instance.certifications, []), 'awards': safe_json_loads(resume_instance.awards, []), 'publications': safe_json_loads(resume_instance.publications, []), 'open_source_contributions': safe_json_loads(resume_instance.open_source_contributions, []), 'interests': safe_json_loads(resume_instance.interests, []), 'references': safe_json_loads(resume_instance.references, []), 'languages': safe_json_loads(resume_instance.languages, {}), 'preferences': safe_json_loads(resume_instance.preferences, {}),
#             'frameworks_tools': safe_json_loads(resume_instance.frameworks_tools, []), 'diploma_details': safe_json_loads(resume_instance.diploma_details, []), 'degree_details': safe_json_loads(resume_instance.degree_details, []), 'certification_details': safe_json_loads(resume_instance.certification_details, []), 'certification_photos': safe_json_loads(resume_instance.certification_photos, []), 'work_preferences': safe_json_loads(resume_instance.work_preferences, []), 'work_authorizations': safe_json_loads(resume_instance.work_authorizations, []), 'professional_links': safe_json_loads(resume_instance.professional_links, []),
...
#             'education_details': { 'tenth': { 'board_name': resume_instance.tenth_board_name, 'school_name': resume_instance.tenth_school_name, 'year_passing': resume_instance.tenth_year_passing, 'score': resume_instance.tenth_score, 'result_upload_url': resume_instance.tenth_result_upload.url if resume_instance.tenth_result_upload else None }, 'twelfth': { 'board_name': resume_instance.twelfth_board_name, 'college_name': resume_instance.twelfth_college_name, 'year_passing': resume_instance.twelfth_year_passing, 'score': resume_instance.twelfth_score, 'result_upload_url': resume_instance.twelfth_result_upload.url if resume_instance.twelfth_result_upload else None }, 'diploma': { 'course_name': resume_instance.diploma_course_name, 'institution_name': resume_instance.diploma_institution_name, 'year_passing': resume_instance.diploma_year_passing, 'score': resume_instance.diploma_score, 'result_upload_url': resume_instance.diploma_result_upload.url if resume_instance.diploma_result_upload else None }, 'degree': { 'degree_name': resume_instance.degree_name, 'institution_name': resume_instance.degree_institution_name, 'specialization': resume_instance.degree_specialization, 'year_passing': resume_instance.degree_year_passing, 'score': resume_instance.degree_score, 'result_upload_url': resume_instance.degree_result_upload.url if resume_instance.degree_result_upload else None } },
#             'resume_pdf_url': resume_instance.resume_pdf.url if resume_instance.resume_pdf else None, 'is_deleted': resume_instance.is_deleted, 'created_at': resume_instance.created_at.isoformat(), 'updated_at': resume_instance.updated_at.isoformat(),
#         }
#         return data
...
#             serialized_data = self._serialize_resume_to_json(resume_instance)
#             return JsonResponse(serialized_data, status=status.HTTP_200_OK)
#         except Resume.DoesNotExist: return JsonResponse({'message': 'Resume not found for this user.'}, status=status.HTTP_404_NOT_FOUND)
#         except Exception as e: return JsonResponse({J: f"An error occurred while retrieving resume: {e}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
#         files = processing_result['files']
#         try:
#             resume_instance, created = Resume.objects.get_or_create(talent_id=user, defaults={'name': data_for_llm_prompt.get('name', getattr(user, 'username', '')), 'email': data_for_llm_prompt.get('email', getattr(user, 'email', '')), 'phone': data_for_llm_prompt.get('phone', getattr(user, 'phone_number', '')), 'is_deleted': False})
#             if resume_instance.is_deleted and not created: resume_instance.is_deleted = False
#             existing_data_for_ai = self._serialize_resume_to_json(resume_instance)
#             merged_data_for_ai = {**existing_data_for_ai, **data_for_llm_prompt}
#             structured_resume = self.ai_pipeline.process_resume_data(merged_data_for_ai, files.get('resume_pdf'), {F: files.get('tenth_result_upload'), G: files.get('twelfth_result_upload'), H: files.get('diploma_result_upload'), C: files.get('degree_result_upload')})
#             self._update_resume_instance(resume_instance, structured_resume, files)
#             resume_instance.save()
#             return JsonResponse({'message': "Resume saved successfully!", 'resume_id': resume_instance.pk}, status=status.HTTP_200_OK)
#         except json.JSONDecodeError as e:
#             raw_llm_output_preview = locals().get('generated_json_text', 'N/A')[:1000]
#             return JsonResponse({J: f"Failed to parse LLM response as JSON: {e}. Preview: {raw_llm_output_preview}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
#         except Exception as e:
#             if '402 Client Error' in str(e) and 'HuggingFace' in str(e): return JsonResponse({J: 'Hugging Face credits exceeded or payment required.'}, status=status.HTTP_402_PAYMENT_REQUIRED)
#             return JsonResponse({J: f"An internal server error occurred: {e}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
#         finally:
#             for path in self.ai_pipeline.get_temp_pdf_paths():
#                 if os.path.exists(path): os.remove(path)

#     def put(self, request, *args, **kwargs): return self.post(request, *args, **kwargs)
#     def patch(self, request, *args, **kwargs): return self.post(request, *args, **kwargs)
#     def delete(self, request, *args, **kwargs):
#         user = request.user
#         try:
#             resume_instance = Resume.objects.get(talent_id=user, is_deleted=False)
...
#             resume_instance.save()
#             return JsonResponse({'message': 'Resume soft-deleted successfully!'}, status=status.HTTP_200_OK)
#         except Resume.DoesNotExist: return JsonResponse({'message': 'Resume not found or already deleted for this user.'}, status=status.HTTP_404_NOT_FOUND)
#         except Exception as e: return JsonResponse({J: f"An error occurred during soft delete: {e}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)


# # The rest of your views for AI analysis etc. are unchanged and fine.
# from .ai_analysis_services import (generate_resume_review, extract_text_from_pdf_path, generate_skill_gap_analysis, generate_career_roadmap)
# class ResumeReviewAPIView(APIView):
#     permission_classes = [IsAuthenticated]
#     def get(self, request, *args, **kwargs):
#         user = request.user
...
#         try:
#             resume_instance = Resume.objects.get(talent_id=user, is_deleted=False)
#             if not resume_instance.resume_pdf: return JsonResponse({'error': 'No resume PDF found. Please upload a resume first.'}, status=status.HTTP_404_NOT_FOUND)
#             pdf_path = os.path.join(settings.MEDIA_ROOT, resume_instance.resume_pdf.name)
#             if not os.path.exists(pdf_path): return JsonResponse({'error': 'Resume file is missing from storage. Please re-upload.'}, status=status.HTTP_404_NOT_FOUND)
#             resume_text = extract_text_from_pdf_path(pdf_path)
#             if not resume_text: return JsonResponse({'error': 'Could not extract text from the resume PDF.'}, status=status.HTTP_400_BAD_REQUEST)
#             review_result = generate_resume_review(resume_text, target_role)
#             return JsonResponse(review_result, status=status.HTTP_200_OK)
#         except Resume.DoesNotExist: return JsonResponse({'error': 'Resume profile not found for this user.'}, status=status.HTTP_404_NOT_FOUND)
#         except Exception as e: return JsonResponse({'error': f'An internal server error occurred: {e}'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
#             resume_instance = Resume.objects.get(talent_id=user, is_deleted=False)
#             resume_skills = json.loads(resume_instance.skills) if resume_instance.skills else []
#             if not resume_skills: return JsonResponse({'error': 'No skills found in your resume profile. Please add skills first.'}, status=status.HTTP_400_BAD_REQUEST)
#             job_roles_on_portal = ["AI Engineer, NLP focus, PyTorch", "Senior Machine Learning Engineer (MLOps)", "Data Scientist with Deep Learning experience", "Research Engineer in Computer Vision", "Generative AI Specialist (LLMs, Diffusion Models)"]
#             skill_gap_result = generate_skill_gap_analysis(resume_skills, job_roles_on_portal)
#             return JsonResponse(skill_gap_result, status=status.HTTP_200_OK)
#         except Resume.DoesNotExist: return JsonResponse({'error': 'Resume profile not found for this user.'}, status=status.HTTP_404_NOT_FOUND)
#         except Exception as e: return JsonResponse({'error': f'An internal server error occurred: {e}'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
#             interests = json.loads(resume_instance.interests) if resume_instance.interests else ["Not specified"]
#             skills = json.loads(resume_instance.skills) if resume_instance.skills else []
#             if not experience_list: return JsonResponse({'error': 'No work experience found in your profile. Please add it to generate a roadmap.'}, status=status.HTTP_400_BAD_REQUEST)
#             current_role = experience_list[0].get('title', 'Not specified')
#             experience_years = len(experience_list) * 1.5
#             roadmap_result = generate_career_roadmap(current_role, experience_years, interests, skills)
#             return JsonResponse(roadmap_result, status=status.HTTP_200_OK)
#         except Resume.DoesNotExist: return JsonResponse({'error': 'Resume profile not found for this user.'}, status=status.HTTP_404_NOT_FOUND)
#         except Exception as e: return JsonResponse({'error': f'An internal server error occurred: {e}'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)