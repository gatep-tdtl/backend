...
def call_llm_api(prompt_text, current_conversation_history=None, output_max_tokens=500):
    """
    Calls the Groq Chat Completions API with the given prompt and an optional conversation history.
    Args:
    ...
    """
    # Use GROQ_API_KEY from config
    if not config.GROQ_API_KEY:
        print("Error: GROQ_API_KEY environment variable is not set. Please set it before running.")
        return None

    # Convert internal chat_history format to Groq API 'messages' format (which is OpenAI compatible)
    messages_for_api = []
    if current_conversation_history:
        for entry in current_conversation_history:
            if "parts" in entry and len(entry["parts"]) > 0:
                # Groq (and OpenAI) expects 'user' and 'assistant' roles
    ...
    headers = {
        ...
        'Authorization': f'Bearer {config.GROQ_API_KEY}' # Use Groq API key
    }

    print(f"\n[AI Processing with Groq ({config.GROQ_MODEL_NAME})...]")
    try:
        response = requests.post(config.GROQ_API_URL, headers=headers, json=payload) # Use Groq API URL
